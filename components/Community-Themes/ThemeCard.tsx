...
function ThemeCard({ theme, image }: Props) {
    ...
    return (
        <div className="p-4 rounded transition duration-300 hover:shadow-lg hover:bg-white hover:bg-opacity-10">
            <h2 className="text-lg font-bold">{theme.data.name}</h2>
            <div className="py-4">
                { image ?
                ...
                <div className="flex justify-between items-center py-4">
                    <div>
                    <span className="mr-4">Created by: {theme.data.username}</span>
                    </div>
                <button onClick={() => {setIsOpen(true)}} className="btn btn-primary px-4 py-2">Download Theme</button>
            </div>
